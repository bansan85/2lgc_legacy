 ...
 */
{
    ...
    path = gtk_tree_model_get_path(GTK_TREE_MODEL(gtk_builder_get_object(projet->list_gtk.ef_appuis.builder, "EF_appuis_treestore")), &appui->Iter_fenetre);
 ...
 */
{
    ...
    while (list_parcours != NULL)
    {
        ...
        gtk_tree_store_append(GTK_TREE_STORE(gtk_builder_get_object(ef_gtk->builder, "EF_appuis_treestore")), &appui->Iter_fenetre, NULL);
        gtk_tree_store_set(GTK_TREE_STORE(gtk_builder_get_object(ef_gtk->builder, "EF_appuis_treestore")), &appui->Iter_fenetre, 0, appui->nom, 1, txt_uxa, 2, txt_uya, 3, txt_uza, 4, txt_rxa, 5, txt_rya, 6, txt_rza, -1);
        free(txt_uxa);
        free(txt_uya);
        free(txt_uza);
        free(txt_rxa);
