 ...
 */
{
    ...
    // M_{Bx} = \frac{\frac{a}{G \cdot J} +k_{Ax}}{\frac{l}{G \cdot J} +k_{Ax}+k_{Bx}} \cdot M_x \textrm{ et } M_{Ax} = M_x - M_{Bx}\end{displaymath}\begin{verbatim}
 ...
 */
{
    ...
    // \varphi_B = -\frac{F_z \cdot a}{6 \cdot E \cdot I \cdot L} \cdot (L^2-a^2)+\frac{M_y}{6 \cdot E \cdot I \cdot L}*(L^2-3 \cdot a^2)\end{displaymath}\begin{verbatim}
 ...
 */
{
    ...
    switch(section_tmp->type)
    {
        ...
        case BETON_SECTION_CIRCULAIRE :
        {
            ...
            if (ERREUR_RELATIVE_EGALE(infos->kBx, MAXDOUBLE))
                BUG(common_fonction_ajout(fonction, a, ll, 0., 0., 0., 0., 0., 0., 0., debut_barre) == 0, -3);
            else
                BUG(common_fonction_ajout(fonction, a, ll, mbx*(infos->kBx + ll/(G*J)), -mbx/(G*J), 0., 0., 0., 0., 0., debut_barre) == 0, -3);
    ...
    // r_x(x) = & -\frac{x}{J \cdot G} \cdot M_{Bx} + M_{Bx} \cdot \left( k_{Bx} + \frac{L}{J \cdot G} \right) & &\textrm{ pour x variant de a à L}\end{align*}\begin{verbatim}
            return 0;
            break;
        }
        default :
 ...
 */
{
    ...
    switch(section_tmp->type)
    {
        ...
        {
            double      I = _1992_1_1_sections_iy(barre->section);
            BUG(common_fonction_ajout(f_rotation, 0., a, -fz*b/(6*E*I*ll)*a*(ll+b),     0.,          fz*b/(2*E*I*ll),       0., 0., 0., 0., debut_barre) == 0, -3);
            BUG(common_fonction_ajout(f_rotation, a,  ll, -fz*a/(6*E*I*ll)*(2*ll*ll+a*a), fz*a/(E*I), -fz*a/(2*E*I*ll),      0., 0., 0., 0., debut_barre) == 0, -3);
            BUG(common_fonction_ajout(f_rotation, 0., a, my/(6*E*I*ll)*(-ll*ll+3*b*b),   0.,          my/(2*E*I*ll),         0., 0., 0., 0., debut_barre) == 0, -3);
            BUG(common_fonction_ajout(f_rotation, a,  ll, my/(6*E*I*ll)*(2*ll*ll+3*a*a),  -my/(E*I),  my/(2*E*I*ll),         0., 0., 0., 0., debut_barre) == 0, -3);
            BUG(common_fonction_ajout(f_rotation, 0., ll, -ll/(6*E*I)*(2*may-mby),      may/(E*I),  -(may+mby)/(2*E*I*ll), 0., 0., 0., 0., debut_barre) == 0, -3);
            
            BUG(common_fonction_ajout(f_deform, 0., a, 0.,                  fz*b/(6*E*I*ll)*(ll*ll-b*b),    0.,                  -fz*b/(6*E*I*ll),     0., 0., 0., debut_barre) == 0, -3);
            BUG(common_fonction_ajout(f_deform, a,  ll, -fz*a*a*a/(6*E*I),  fz*a/(6*E*I*ll)*(a*a+2*ll*ll),  -fz*a/(2*E*I),      fz*a/(6*E*I*ll),      0., 0., 0., debut_barre) == 0, -3);
            BUG(common_fonction_ajout(f_deform, 0., a, 0.,                  my/(6*E*I*ll)*(ll*ll-3*b*b),    0.,                  -my/(6*E*I*ll),       0., 0., 0., debut_barre) == 0, -3);
            BUG(common_fonction_ajout(f_deform, a,  ll, my/(6*E*I)*(3*a*a), -my/(6*E*I*ll)*(2*ll*ll+3*a*a), my/(6*E*I*ll)*(3*ll), -my/(6*E*I*ll),       0., 0., 0., debut_barre) == 0, -3);
 ...
 */
{
    ...
    switch(section_tmp->type)
    {
        ...
        {
            double      I = _1992_1_1_sections_iz(barre->section);
            BUG(common_fonction_ajout(f_rotation, 0., a, fy*b/(6*E*I*ll)*a*(ll+b),     0.,          -fy*b/(2*E*I*ll),       0., 0., 0., 0., debut_barre) == 0, -3);
            BUG(common_fonction_ajout(f_rotation, a,  ll, fy*a/(6*E*I*ll)*(2*ll*ll+a*a), -fy*a/(E*I), fy*a/(2*E*I*ll),      0., 0., 0., 0., debut_barre) == 0, -3);
            BUG(common_fonction_ajout(f_rotation, 0., a, mz/(6*E*I*ll)*(-ll*ll+3*b*b),   0.,          mz/(2*E*I*ll),         0., 0., 0., 0., debut_barre) == 0, -3);
            BUG(common_fonction_ajout(f_rotation, a,  ll, mz/(6*E*I*ll)*(2*ll*ll+3*a*a),  -mz/(E*I),  mz/(2*E*I*ll),         0., 0., 0., 0., debut_barre) == 0, -3);
            BUG(common_fonction_ajout(f_rotation, 0., ll, -ll/(6*E*I)*(2*maz-mbz),      maz/(E*I),  -(maz+mbz)/(2*E*I*ll), 0., 0., 0., 0., debut_barre) == 0, -3);
            
            BUG(common_fonction_ajout(f_deform, 0., a, 0.,                  fy*b/(6*E*I*ll)*(ll*ll-b*b),    0.,                  -fy*b/(6*E*I*ll),     0., 0., 0., debut_barre) == 0, -3);
            BUG(common_fonction_ajout(f_deform, a,  ll, -fy*a*a*a/(6*E*I),  fy*a/(6*E*I*ll)*(a*a+2*ll*ll),  -fy*a/(2*E*I),      fy*a/(6*E*I*ll),      0., 0., 0., debut_barre) == 0, -3);
            BUG(common_fonction_ajout(f_deform, 0., a, 0.,                  mz/(6*E*I*ll)*(-ll*ll+3*b*b),    0.,                  mz/(6*E*I*ll),       0., 0., 0., debut_barre) == 0, -3);
            BUG(common_fonction_ajout(f_deform, a,  ll, -mz/(6*E*I)*(3*a*a), mz/(6*E*I*ll)*(2*ll*ll+3*a*a), -mz/(6*E*I*ll)*(3*ll), mz/(6*E*I*ll),       0., 0., 0., debut_barre) == 0, -3);
 ...
 */
{
    ...
    // f_x(x) = \frac{a \cdot (F_{Ax} + F_{Bx}) - F_{Bx} \cdot x}{E \cdot S} \textrm{ pour x variant de a à l}\end{displaymath}\begin{verbatim}
            ...
            BUG(common_fonction_ajout(fonction, a,  ll, a*(fax+fbx)/(E*S), -fbx/(E*S), 0., 0., 0., 0., 0., debut_barre) == 0, -3);
            return 0;
            break;
        }
        default :
